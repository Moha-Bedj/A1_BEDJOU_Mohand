...
public class Analyseu_Syuntaxique {
    private Analyseur_Lexical lexer;
    private tokens currentToken;

    private List<String> erreurs = new ArrayList<>();
    private HashSet<String> tableSymboles = new HashSet<>();

    public Analyseu_Syuntaxique(Analyseur_Lexical lexer) {
        this.lexer = lexer;
    ...
    private void erreurSemantique(String msg) {
        erreurs.add("Erreur sémantique ligne " + currentToken.ligne + " : " + msg);
    }

    // Méthode pour ignorer un bloc complet, y compris parenthèses et accolades imbriquées
    private void Skip() {
    ...
    private void INSTR() {
        switch (currentToken.type) {
            ...
            case TRY -> TRY_CATCH();
            case IF, ELSE, BEDJOU, MOHAND, DO, WHILE -> Skip();
            default -> erreur("Instruction inattendue");
        }
    }

    private void DECL() {
        currentToken = lexer.getNextToken();
        if (tableSymboles.contains(currentToken.lexeme)) erreurSemantique("Variable '" + currentToken.lexeme + "' déjà déclarée.");
        else tableSymboles.add(currentToken.lexeme);
        if (currentToken.type == tokentype.IDENTIFIANT) currentToken = lexer.getNextToken();
    ...
    private void AFFECT() {
        if (!tableSymboles.contains(currentToken.lexeme)) erreurSemantique("Variable '" + currentToken.lexeme + "' non déclarée.");
        if (currentToken.type == tokentype.IDENTIFIANT) currentToken = lexer.getNextToken();
